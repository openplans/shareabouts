...
describe Profile do
  ...
  describe "self.find_or_create_by_request_fingerprint" do
    attr_reader :request
    
    context "when existing profile for fingerprint" do
      before do
        @request = OpenStruct.new :env => { 'HTTP_USER_AGENT' => profile.user_agent }, :remote_ip => profile.client_ip
      end
      
      it "returns existing profile" do
        Profile.find_or_create_by_request_fingerprint( request ).should == profile
      end
    end
    
    context "when no existing profile for fingerprint" do
      before do
      ...
      it "creates a new profile" do
        lambda {
          Profile.find_or_create_by_request_fingerprint( request )
        }.should change(Profile, :count).by(1)
      end
      
      it "returns newly created profile" do
        profile = Profile.find_or_create_by_request_fingerprint( request )
